...
export function PropertyMediaCarousel({
  ...
  if (images.length === 0) {
    return (
      <div
        className={cn('flex aspect-4/3 w-full items-center justify-center bg-border', className)}
      >
      ...
      <Carousel showDots={showDots}>
        {images.map((img, i) => (
          <img
            ...
            className={cn('aspect-4/3.5 w-full object-cover', onOpenGallery && 'cursor-pointer')}
          />
        ))}
      </Carousel>
